	...
	public class GraphRenderer : IDisposable
	{
		...
		public GraphRenderer(IViewport control)
		{
			...
			_nodeRenderer = new NodeRenderer(
				_baseNodePaint,
				SKTypeface.FromFamilyName("Segoe UI", SKFontStyleWeight.Medium, SKFontStyleWidth.Normal,
					SKFontStyleSlant.Upright),
				SKTypeface.FromFamilyName("Segoe UI", SKFontStyleWeight.Normal, SKFontStyleWidth.Normal,
					SKFontStyleSlant.Upright)
			);
		}
						...
						if (IsShiftKeyDown)
						{
							var connections = _control.Graph
								.SelectMany(n => n
									.Connections
									.Where(connection =>
										connection.Source == pin.PinId || connection.Destination == pin.PinId)
								).ToArray();
		...
		public void OnRender()
		{
			...
			using (new SKAutoCanvasRestore(_canvas, true))
			{
				...
				if (_rectangleSelecting)
					_canvas.DrawRect(_mouseDownPos.X, _mouseDownPos.Y, _lastMouseBoardPos.X - _mouseDownPos.X,
						_lastMouseBoardPos.Y - _mouseDownPos.Y, _selectionBoxPaint);
		...
		private void DrawConnections(IPin hotPin, SKPaint ephemeralConnectionPaint, SKPaint connectionPaint,
			SKPaint deleteConnectionPaint)
		{
			...
			foreach (var connection in node.Connections)
			{
				...
				if (hotPin != null && (inputPin.Pin.PinId == hotPin.PinId || outputPin.Pin.PinId == hotPin.PinId) &&
				    IsShiftKeyDown)
			...
			if (_dragSourcePin != null)
			{
				...
				if (hotPin != null && _dragSourcePin.Pin.CanConnectTo(hotPin) &&
				    hotPin.PinId.Node != _dragSourcePin.Node.NodeId)
				{
		...
		{
			return _control.Graph.LastOrDefault(node =>
				_nodeRenderer.GetBounds(node).Contains(new Vector2(_lastMouseBoardPos.X, _lastMouseBoardPos.Y)));
		}
